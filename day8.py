...
def remove_vegetable(my_vegetables):
    ...
    num_input = input("削除する番号を入力してください >>").strip()
    ...
    num = int(num_input)
    if num < 1 or num > len(my_vegetables):
        print("⚠️  数字を入力してください")
        return
...
def register_planting(my_vegetables):
    ...
    num_input = input("番号を入力してください >> ").strip()
    if not num_input.isdigit() or not (1 <= int(num_input) <= len(my_vegetables)):
        print("⚠️ 正しい番号を入力してください")
        return

    veg = my_vegetables[int(num_input) - 1]
    ...
    if veg in GROW_DAYS:
        ...
        harvest_date_only = harvest_date.date()

        #残り日数の計算（予定日　-　今日）
        remaining = (harvest_date_only - today).days
