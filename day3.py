...
if save.lower() == "y":
    ...
    with open("shopping_list.txt", "w", encoding="utf-8")as f:
        f.write(f"作成日時: {date_str}\n\n")
        f.write(f"【{recipe_name} {guests}】人分の材料リスト】\n\n")

        #スパイスカレーの場合
        if choice == 1:
            f.write(f"・玉ねぎ：{onions * rate:.1f} 個\n")
            f.write(f"・トマト缶:{tomato * rate:.1f} g\n")
            f.write(f"・鶏肉:{chicken * rate:.1f} g\n")
            f.write(f"・ターメリック:{turmeric * rate:.1f} g\n")
            f.write(f"・クミン:{cumin * rate:.1f} g\n")
            f.write(f"・コリアンダー:{coriander* rate:.1f} g\n")
            f.write(f"・ショウガ:{ginger* rate:.1f} g\n")
            f.write(f"・ニンニク:{garlic* rate:.1f} g\n")
            f.write(f"・お水:{water * rate:.1f} ml\n")
            f.write(f"・砂糖:{sugar* rate:.1f} g\n")
            f.write(f"・醤油:{soy_sauce* rate:.1f} g\n")
            f.write(f"・ヨーグルト:{yoghurt* rate:.1f} g\n")
            f.write(f"・塩:{salt* rate:.1f} g\n")
        
        #肉じゃがの場合
        elif choice == 2:
            f.write(f"・牛薄切り肉（または豚バラ肉）：{beef * rate:.1f} g\n")
            f.write(f"・ジャガイモ:{potato * rate:.1f} 個\n")
            f.write(f"・玉ねぎ:{onions * rate:.1f} 個\n")
            f.write(f"・にんじん:{carrot * rate:.1f} 本\n")
            f.write(f"・しらたき:{shirataki * rate:.1f} g\n")
            f.write(f"・だし汁:{stock * rate:.1f} ml\n")
            f.write(f"・砂糖:{sugar * rate:.1f} g\n")
            f.write(f"・酒:{sake * rate:.1f} g\n")
            f.write(f"・みりん:{mirin * rate:.1f} g\n")
            f.write(f"・醤油:{soy_sauce* rate:.1f} g\n")

        #ハンバーグの場合
        elif choice == 3:
            f.write(f"・合挽きひき肉：{beef_pork_mince * rate:.1f} g\n")
            f.write(f"・玉ねぎ:{onions * rate:.1f} 個\n")
            f.write(f"・パン粉:{bread_crumbs * rate:.1f} g\n")
            f.write(f"・牛乳:{milk* rate:.1f} ml\n")
            f.write(f"・卵:{egg * rate:.1f} 個\n")
            f.write(f"・ナツメグ:{nutmeg * rate:.1f} g\n")
            f.write(f"・塩:{salt * rate:.1f} g\n")
            f.write(f"・胡椒:{pepper * rate:.1f} g\n")

    print(f"\n✅ shopping_list.txt に保存しました！")
